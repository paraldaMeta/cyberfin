...
### P1 (天机荐股系统 - 2026-03-11 新增)
- ✅ 每日日柱计算（基于儒略日算法）
- ✅ 财神方位计算（阳贵人/阴贵人+流年飞星叠加）
- ✅ 时辰吉凶排序（日干×时辰六神关系）
- ✅ 五行强度评级（双旺/旺/平/衰/极衰）
- ✅ 特殊日期识别（奔驰骏马大吉日/跛行之马凶日）
- ✅ 天机荐股推荐算法（喜用神×今日五行×月度评级）
- ✅ 首页天机板块UI（今日天机横幅、财神方位、最佳时辰、五行气场、天机荐股卡片）
- ✅ 月度天机日历API
- ✅ 个性化推荐（登录用户专属）
...
### 天机荐股模块 (2026-03-11 新增)
- `/app/backend/services/bazi/daily_tianji.py` - 每日天机计算核心
- `/app/frontend/src/components/TianjiPanel.jsx` - 天机面板组件

### 天机API
- `/api/tianji/today` - 获取今日天机数据
- `/api/tianji/today?user_id=xxx` - 获取个性化天机推荐
- `/api/tianji/date/{date}` - 获取指定日期天机数据
- `/api/tianji/calendar/{year}/{month}` - 获取月度天机日历
